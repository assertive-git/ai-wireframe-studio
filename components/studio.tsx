...
type View =
  "dashboard" | "intake" | "hearing" | "generating" | "workspace" | "settings";
...
export default function Studio() {
  ...
  async function formatCode() {
    try {
      ...
      setDocumentState((current) =>
        current.html !== html || current.css !== css
          ? current
          : { ...current, ...formatted, previewHtml: formatted.html }
      );
    } catch {
      setError(
        "コードを整形できませんでした。HTML/CSSの構文を確認してください。"
      );
    }
  }
  ...
  function addPendingFiles(kind: "reference" | "usable", files: File[]) {
    ...
    const setter =
      kind === "reference" ? setPendingReferenceFiles : setPendingUsableFiles;
          ...
          <div className="form-grid">
            ...
            <div className="field full">
              ...
              >
                ...
                />
                <small>
                  AIが構成・デザインの参考として確認します（HTMLには使用しません）。
                </small>
                    ...
                    <div className="upload-list">
                      {pendingUsableFiles.map((file, index) => (
                        ...
                        >
                          <span>{file.name}</span>
                          <span>
                            {Math.ceil(file.size / 1024)} KB ・ 使用可
                          </span>
                        </div>
                      ))}
                    </div>
