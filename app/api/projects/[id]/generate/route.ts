...
export async function POST(_: Request, { params }: Params) {
  ...
  if (!process.env.OPENAI_API_KEY) {
    ...
    const usableAssets = project.assets.filter(
      (asset) => asset.rights === "usable"
    );
...
${JSON.stringify(usableAssetUrls, null, 2)}`;
    try {
      if (prompt.length > 200_000) {
        throw new GeneratedPageError(
          "資料と会話の合計が大きすぎます。不要な資料を整理して再実行してください。必須要件は省略していません。"
        );
      }
      ...
      return NextResponse.json(
        {
          error:
            "AIサービスへのリクエストに失敗しました。しばらくしてからお試しください。",
        },
        { status: 502 }
      );
    }
  ...
  Object.assign(
    generated,
    await formatGeneratedSource(generated.html, generated.css)
  );
